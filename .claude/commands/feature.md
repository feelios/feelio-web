---
description: "Feelio 프론트엔드 기능(Feature)을 하네스+루프로 구현 (사용법 /feature F4-1)"
argument-hint: "<이슈코드 예: F4-1>"
---

# Claude Code 슬래시 커맨드 — Gemini CLI 의 .agents/skills/feature/SKILL.md 와 동일 동작.
# 두 파일은 항상 같은 내용을 유지한다(도구만 다름): $ARGUMENTS ↔ {{args}} 만 차이.

너는 Feelio 프론트엔드 시니어 개발자다. 인자로 받은 **기능(Feature) 하나만** 처리한다: $ARGUMENTS
다른 기능 범위나 공통 컴포넌트는 절대 임의로 건드리지 않는다.

## 0. 기능 식별
**`docs/ISSUES.md`의 프론트엔드 기능 이슈 표(SSOT)** 에서 `$ARGUMENTS`(예: F4-1)에 해당하는 행을 찾아
브랜치·계약(§)·계층·캐시키·상태·완료기준을 확정한다.
표에 없거나 인자가 비었으면 추측하지 말고 사용자에게 어떤 기능인지 먼저 물어라.
> 기능 이슈 표는 Claude/Gemini 공통 기준이라 `docs/ISSUES.md` 한 곳에서만 관리한다(여기에 중복 정의하지 않음).
...
- 아키텍처 구조: `src/pages`, `src/components`, `src/hooks`, `src/api`, `src/stores` 분리 준수
- API 통신: `src/api/client.js`의 Axios 인스턴스 사용 (엔드포인트 및 DTO는 `docs/API-CONTRACT.md` 절대 준수)
- 상태 및 캐싱: 서버 상태는 TanStack Query(표에 명시된 Query Key 배열 정확히 일치), 전역 클라이언트 상태는 Zustand(`src/stores/`) 사용
...
5) 슬롯 순서로 구현(API 함수(src/api/) → Custom Hook(useQuery/useMutation) → UI Component → Page 연결)
6) `npm run lint` 및 `npm run build` 사전 검증 (브라우저/콘솔 에러 확인)
7) 실패 시 (규칙위반/계약불일치/논리오류)로 분류 → 최소 수정 → 재검증
8) lint·build 가 모두 통과되면 작업을 마무리한다. (통과 전에는 "완료"라고 하지 않는다)
9) [이슈/PR 템플릿 출력]: 작업이 완료되면 사용자가 GitHub에 바로 복사/붙여넣기 할 수 있도록 아래 형식의 마크다운 템플릿을 생성하여 출력한다.
   - Title: `[$ARGUMENTS] feat/fix: 작업한 핵심 내용 요약`
...
10) [Git 자동화 명령어 제공]: 9번의 템플릿 출력이 끝나면, 사용자가 바로 커밋과 푸시를 할 수 있도록 아래의 Git 명령어를 코드 블록으로 제공한다. 커밋 메시지는 9번의 Title과 정확히 일치해야 한다.
    ```bash
    git add .
    git commit -m "[$ARGUMENTS] feat/fix: 작업한 핵심 내용 요약"
