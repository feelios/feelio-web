...
- **기준은 말랑이 몸 색(`blob`)이다. `color`·`light` 를 거기 맞추며, 반대로 하지 않는다.**
  ...
  - 신남이 오래 어긋나 있었다. `color` 는 주황인데 말랑이는 분홍이라, 같은 감정이 화면
    위치마다 다른 색으로 보였다. 말랑이 계열로 정리했다 (#291).
  - 나머지 7종은 `color` 가 말랑이 몸 색보다 **한 톤 진한 같은 계열**이다.
    신남은 몸 색(`#FF9DC4`)을 그대로 쓰면 설렘(`#F28AB7`)과 붙어 차트에서 구분이 어렵다.
    말랑이가 분홍→살구(`#FFC49A`) 그라데이션이므로 살구 쪽으로 살짝 당겨 `#FF8FA3` 을 쓴다.
    말랑이 안에 있는 색이라 캐릭터와 어긋나지 않으면서 설렘과는 벌어진다.
