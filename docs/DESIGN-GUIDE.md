...
- **기준은 말랑이 몸 색(`blob`)이다. `color`·`light` 를 거기 맞추며, 반대로 하지 않는다.**
  말랑이는 서비스의 얼굴이라 사용자가 색으로 기억한다. 칩·차트가 어긋나면 그쪽을 고친다.
  - 신남이 오래 어긋나 있었다. `color` 는 주황인데 말랑이는 분홍이라, 같은 감정이 화면
    위치마다 다른 색으로 보였다. 말랑이 색(`#FF9DC4`)에 맞춰 정리했다 (#291).
  - 나머지 7종은 `color` 가 말랑이 몸 색보다 **한 톤 진한 같은 계열**이다. 신남은 그렇게 잡으면
    설렘(`#F28AB7`)과 겹쳐 차트에서 구분되지 않아 몸 색과 같은 값을 쓴다.
- **감정 색은 반드시 `getEmotion(감정명)` 또는 theme.js 경유로 사용한다. 컴포넌트에 hex 하드코딩 금지.**
- `GET /api/meta` 에서 받는 것은 **신원(emotionId·이름·정렬)뿐이다. 색은 여기 팔레트가 기준이다.**
  서버 `color` 를 그대로 쓰면 DB 시드 값(신남 `#FF8A62`)이 나와 디자인과 어긋난다.
  `mergeEmotions()`(`src/constants/metadata.js`)가 신원은 서버, 색은 로컬로 합친다.
  - emotionId 는 DB 가 만드는 값이라 프론트가 가질 수 없다. 환경(KEC/KMJ/NPR)마다 다르고
    마스터를 다시 시드할 때마다 바뀐다. 하드코딩했다가 저장이 전부 실패한 적이 있다 (#266).
