...
| F6-1 | 로컬 스토리지 보안 최적화 | `refactor/storage-whitelist` | §13 | Store·Hook | — | 신규 | §13 민감/비즈니스 데이터 스토리지에서 제거 + UI 상태(themeMode, aurora 등)만 로컬 유지(partialize 적용) |
| F6-2 | Access 토큰 메모리 관리 | `feat/auth-memory` | §14 | api·Store | — | 신규 | §14 (기존 F1-1 확장) Access 토큰을 로컬 스토리지에서 완전 배제하고 메모리(Zustand) 상태로만 관리하도록 개편 |
| F6-3 | 모의 데이터(Mock) 전면 제거 | `refactor/data-fetching` | §15 | Page·Hook | `['tx']`, `['goals']` | 신규 | §15 로컬 스토리지 의존성 100% 제거 + HomePage 등 남은 화면 컴포넌트 마운트 시 API 패칭(React Query)으로 완전 전환 |
