...
```
- `totalAsset`: 사용자의 현재 전체 자산(원, 정수 ≥ 0). 온보딩에서 입력받아 저장되며 홈 총자산 카드(F7-7) 등에서 사용. 미설정 시 `0`.

### PATCH /api/users/me · 인증 필요
Request: `{ "nickname": "새닉네임", "totalAsset": 5000000 }` (부분 전송 허용 — nickname 1~8자, totalAsset 정수 ≥ 0) → Response `data`: 갱신된 user 객체. 에러: VALIDATION_ERROR
...
```
- 기록 없는 날짜는 배열에서 생략.
- 대표 감정은 **건수 → 지출 금액** 순으로 정한다(둘 다 같으면 최근 기록). 홈의 대표 말랑이·감정 능선과 같은 규칙이다 —
  하루에 감정을 한 건씩 적으면 전부 1건이라, 최근 기록으로 가르면 마지막에 적은 감정이 늘 그날의 색이 된다.
